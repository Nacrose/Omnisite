...
const NOTIFICATIONS: Notification[] = [
  ...
  { id: 'n8', type: 'approval', title: 'RA Bill #4 submitted', desc: 'NPR 18.4 Cr — awaiting client approval', time: 'Yesterday', unread: false, severity: 'info' },
]
        ...
        <div className="absolute right-0 top-full mt-1 w-[380px] pane border border-[var(--pane-divider)] rounded-lg shadow-2xl z-50 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 h-11 border-b border-[var(--pane-divider)]">
            <div className="flex items-center gap-2">
              ...
              {unreadCount > 0 && (
                <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-[var(--critical)] text-white font-semibold">
                  {unreadCount} new
                </span>
              )}
            </div>
          ...
          <div className="px-4 py-2 border-t border-[var(--pane-divider)] bg-secondary/20">
            <button className="w-full text-center text-[11px] text-primary hover:underline">
              View all in Correspondence →
            </button>
          </div>
        </div>
      )}
