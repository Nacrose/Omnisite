...
export function BillingHoldsView() {
  ...
  const release = async (id: string) => {
    try {
      const res = await fetch(`/api/billing-holds?id=${id}`, { method: 'PATCH',
  ...
  return (
    <>
      <PaneHeader title="Billing Holds">
        <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={fetchHolds} disabled={loading}>
          {loading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : 'Refresh'}
        </Button>
      </PaneHeader>
      <PaneBody className="p-4">
        {holds.length > 0 && (
          <div className={cn('mb-3 rounded-md p-3 text-xs',
            totalLocked > 0 ? 'border border-amber-500/30 bg-amber-500/10' : 'border border-emerald-500/30 bg-emerald-500/10'
          )}>
            <div className="flex items-center gap-1.5 font-medium">
              {totalLocked > 0 ? <AlertTriangle className="h-3.5 w-3.5 text-amber-500" /> : <Lock className="h-3.5 w-3.5 text-emerald-500" />}
        ...
        {holds.length === 0 && !loading && (
          <div className="text-muted-foreground flex flex-col items-center justify-center gap-2 py-12 text-center">
            ...
            <p className="text-muted-foreground max-w-xs text-[11px]">
              Billing holds are created automatically when NCRs are opened. They block vendor payments until the NCR is closed.
            </p>
          </div>
        )}

        {holds.map((h) => (
          <div key={h.id} className="mb-1.5 flex items-center gap-3 rounded border border-[var(--pane-divider)] p-2">
            ...
            <Button size="sm" variant="outline" className="h-6 gap-1 px-2 text-[10px]" onClick={() => release(h.id)}>
