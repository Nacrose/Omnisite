...
export function WorkProgressView({ entries, selectedId, onSelect }: {
  ...
  const { visible, isVisible, toggle } = useColumnVisibility(COLS.map(c => c.key), [], 'work-progress')
  return (
    <>
        ...
        <StickyTableBody>
          {entries.map(d => {
            ...
            return (
              ...
              >
                {isVisible('dsr') && <div className="w-20 px-2 font-mono text-muted-foreground">{d.id}</div>}
                {isVisible('task') && (
                ...
                {isVisible('chainage') && <div className="w-32 px-2 font-mono text-[10px] text-muted-foreground truncate">{d.chainage}</div>}
                {isVisible('planned') && <div className="w-20 px-2 text-right font-mono">{d.planned || '—'}</div>}
                {isVisible('actual') && (
                ...
                {isVisible('uom') && <div className="w-14 px-2 text-muted-foreground">{d.uom}</div>}
                {isVisible('status') && <div className="w-28 px-2"><StatusDot status={d.status} /></div>}
                {isVisible('actions') && (
