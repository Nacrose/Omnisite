 ...
 * - Single letter keys (when not typing in an input) → switch module
 * - N → Quick Add menu
 * - [ → Toggle left pane
 * - ] → Toggle right pane
 */
export function useKeyboardShortcuts() {
  const { setActiveModule, setQuickAddOpen, toggleLeftPane, toggleRightPane } = useApp()
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (KEYBOARD_SHORTCUTS[key]) {
        e.preventDefault()
        setActiveModule(KEYBOARD_SHORTCUTS[key])
      }
    }
    document.addEventListener('keydown', handler)
    return () => document.removeEventListener('keydown', handler)
  }, [setActiveModule, setQuickAddOpen, toggleLeftPane, toggleRightPane])
}
