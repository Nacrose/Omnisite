...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    logDbError('purchase_orders', 'GET', error, { userId: user.id })
...
export async function POST(req: NextRequest) {
  ...
  if (!oldData && isProjectScopedTable('purchase_orders')) {
  ...
  if (error || !data) {
    logDbError('purchase_orders', 'POST', error || 'no data returned', {
...
export async function DELETE(req: NextRequest) {
  ...
  const { deleted, error } = await deleteWithAudit('purchase_orders', id, 'id', user.id)

  if (error) {
    logDbError('purchase_orders', 'DELETE', error, { recordId: id, userId: user.id })
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
