...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    logDbError('letters', 'GET', error, { userId: user.id })
...
export async function POST(req: NextRequest) {
  ...
  if (!oldData && isProjectScopedTable('letters')) {
  ...
  if (error || !data) {
    logDbError('letters', 'POST', error || 'no data returned', {
...
export async function DELETE(req: NextRequest) {
  ...
  const userClient = createUserClient(user.accessToken)
  const { data: existing } = await userClient.from('letters').select('id').eq('id', id).limit(1)
  ...
  const { deleted, error } = await deleteWithAudit('letters', id, 'id', user.id)

  if (error) {
    logDbError('letters', 'DELETE', error, { recordId: id, userId: user.id })
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
