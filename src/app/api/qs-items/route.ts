...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    logDbError('qs_items', 'GET', error, { userId: user.id })
...
export async function POST(req: NextRequest) {
  ...
  if (!oldData && isProjectScopedTable('qs_items')) {
  ...
  if (error || !data) {
    logDbError('qs_items', 'POST', error || 'no data returned', {
...
export async function DELETE(req: NextRequest) {
  ...
  const userClient = createUserClient(user.accessToken)
  const { data: existing } = await userClient.from('qs_items').select('id').eq('id', id).limit(1)
  ...
  const { deleted, error } = await deleteWithAudit('qs_items', id, 'id', user.id)

  if (error) {
    logDbError('qs_items', 'DELETE', error, { recordId: id, userId: user.id })
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
