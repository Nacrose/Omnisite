...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    logDbError('dsr_entries', 'GET', error, { userId: user.id })
...
export async function POST(req: NextRequest) {
  ...
  if (!oldData && isProjectScopedTable('dsr_entries')) {
  ...
  if (error || !data) {
    logDbError('dsr_entries', 'POST', error || 'no data returned', {
...
export async function DELETE(req: NextRequest) {
  ...
  const userClient = createUserClient(user.accessToken)
  const { data: existing } = await userClient.from('dsr_entries').select('id').eq('id', id).limit(1)
  ...
  const { deleted, error } = await deleteWithAudit('dsr_entries', id, 'id', user.id)

  if (error) {
    logDbError('dsr_entries', 'DELETE', error, { recordId: id, userId: user.id })
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
