...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    logDbError('equipment', 'GET', error, { userId: user.id })
...
export async function POST(req: NextRequest) {
  ...
  if (!oldData && isProjectScopedTable('equipment')) {
  ...
  if (error || !data) {
    logDbError('equipment', 'POST', error || 'no data returned', {
...
export async function DELETE(req: NextRequest) {
  ...
  const userClient = createUserClient(user.accessToken)
  const { data: existing } = await userClient.from('equipment').select('id').eq('id', id).limit(1)
  ...
  const { deleted, error } = await deleteWithAudit('equipment', id, 'id', user.id)

  if (error) {
    logDbError('equipment', 'DELETE', error, { recordId: id, userId: user.id })
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
