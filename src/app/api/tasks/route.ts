...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    logDbError('tasks', 'GET', error, { userId: user.id })
...
export async function POST(req: NextRequest) {
  ...
  if (!oldData && isProjectScopedTable('tasks')) {
  ...
  if (error || !data) {
    logDbError('tasks', 'POST', error || 'no data returned', {
...
export async function DELETE(req: NextRequest) {
  ...
  const userClient = createUserClient(user.accessToken)
  const { data: existing } = await userClient.from('tasks').select('id').eq('id', id).limit(1)
  ...
  const { deleted, error } = await deleteWithAudit('tasks', id, 'id', user.id)

  if (error) {
    logDbError('tasks', 'DELETE', error, { recordId: id, userId: user.id })
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
