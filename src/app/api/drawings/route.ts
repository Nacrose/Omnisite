...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    logDbError('drawings', 'GET', error, { userId: user.id })
...
export async function POST(req: NextRequest) {
  ...
  if (!oldData && isProjectScopedTable('drawings')) {
  ...
  if (error || !data) {
    logDbError('drawings', 'POST', error || 'no data returned', {
...
export async function DELETE(req: NextRequest) {
  ...
  const userClient = createUserClient(user.accessToken)
  const { data: existing } = await userClient.from('drawings').select('id').eq('id', id).limit(1)
  ...
  const { deleted, error } = await deleteWithAudit('drawings', id, 'id', user.id)

  if (error) {
    logDbError('drawings', 'DELETE', error, { recordId: id, userId: user.id })
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
