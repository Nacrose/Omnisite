...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    console.error('[API] subcontractors error:', error)
...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await userClient.from('subcontractors').upsert(body).select()
...
export async function DELETE(req: NextRequest) {
  ...
  // Use a user-scoped client so RLS policies are enforced.
  const userClient = createUserClient(user.accessToken)
  const { error } = await userClient.from('subcontractors').delete().eq('id', id)
